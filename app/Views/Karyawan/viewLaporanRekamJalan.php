...
<body class="hold-transition sidebar-mini">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                	<div style="padding-bottom: 20px;">
                                        <form method="post" action="<?= base_url('') ?>/Karyawan/LaporanRekamJalan/data_cetak">
    				                		<div class="row">
    				                			<div class="col-md-5">
    ...
    <script>
        function ganti(poli) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanRekamJalan/data/' + $('#tanggal').val() + '/' + poli).load();
        };
	    ...
	    $(function() {
            ...
            $("#input_pasien").select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?= base_url('Karyawan/LaporanRekamJalan/data_pasien'); ?>',
                    type: "post",
                    delay: 250,
                    dataType: 'json',
	        ...
	        $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
	            ...
	            $('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanRekamJalan/data/' + tanggal).load();
	        });
	        $("#btn_reset").click(function (e) {
	            $("#input_pasien").val('').trigger('change')
            	$('.table').DataTable().ajax.url('<?= base_url() ?>/Karyawan/LaporanRekamJalan/data/' + $('#tanggal').val() + '/' + $("#input_pasien").val()).load();
	        });
	        ...
	        $(".table").DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Karyawan/LaporanRekamJalan/data/" + $('#tanggal').val() + '/' + $('#input_pasien').val(),
                    "dataSrc": ""
                },
                "columns": [{
                        "data": "nama_pasien"
