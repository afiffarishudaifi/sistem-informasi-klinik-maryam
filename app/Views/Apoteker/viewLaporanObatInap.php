...
<body class="hold-transition sidebar-mini">
    ...
    <script>
        function ganti(obat) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Apoteker/LaporanObatInap/data/' + $('#tanggal').val() + '/' + obat).load();
        };
	    ...
	    $(function() {
            ...
            $("#input_obat").select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?php echo base_url('Apoteker/LaporanObatInap/data_obat'); ?>',
                    type: "post",
                    delay: 250,
                    dataType: 'json',
	        ...
	        $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
	            ...
	            $('.table').DataTable().ajax.url('<?= base_url() ?>/Apoteker/LaporanObatInap/data/' + tanggal + '/' + $('#input_obat').val()).load();
	        });

            $("#btn_reset").click(function (e) {
	            $("#input_obat").val('').trigger('change')
                $('.table').DataTable().ajax.url('<?= base_url() ?>/Apoteker/LaporanObatInap/data/' + $('#tanggal').val() + '/' + $('#input_obat').val()).load();
            });
	        ...
	        $(".table").DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Apoteker/LaporanObatInap/data/" + $('#tanggal').val() + '/' + $('#input_obat').val(),
                    "dataSrc": ""
                },
                "columns": [{
                        "data": "nama_obat"
