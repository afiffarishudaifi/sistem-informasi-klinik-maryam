...
<body class="hold-transition sidebar-mini">
    ...
    <script>
        function ganti(dokter) {
            $('.table').DataTable().ajax.url('<?= base_url() ?>/Pasien/RekamMedisInap/data/' + $('#tanggal').val() + '/' + dokter).load();
        };
        ...
        $(function() {
            ...
            $("#input_pasien").select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?php echo base_url('Pasien/RekamMedisInap/data_pasien'); ?>',
                    type: "post",
                    delay: 250,
                    dataType: 'json',
            ...
            $("#input_dokter").select2({
                ...
                theme: 'bootstrap4',
                ajax: {
                    url: '<?php echo base_url('Pasien/RekamMedisInap/data_dokter'); ?>',
                    type: "post",
                    delay: 250,
                    dataType: 'json',
                    data: function(params) {
            ...
            $('#tanggal').on('apply.daterangepicker', function(ev, picker) {
                ...
                $('.table').DataTable().ajax.url('<?= base_url() ?>/Pasien/RekamMedisInap/data/' + tanggal + '/' + $('#input_dokter').val()).load();
            });

            $("#btn_reset").click(function (e) {
                $("#input_pasien").val('').trigger('change')
                $("#input_dokter").val('').trigger('change')
                $('.table').DataTable().ajax.url('<?= base_url() ?>/Pasien/RekamMedisInap/data/' + $('#tanggal').val() + '/' + $('#input_dokter').val()).load();
            });
            ...
            $(".table").DataTable({
                ...
                "ajax": {
                    "url": "<?= base_url() ?>/Pasien/RekamMedisInap/data/" + $('#tanggal').val() + $('#input_dokter').val(),
                    "dataSrc": ""
                },
                "columns": [{
                        "data": "nama_pasien"
